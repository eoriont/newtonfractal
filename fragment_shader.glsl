...
vec4 newtonsMethod(complex c) {
  ...
  // Make color based on zn
  float hue = 0.5 + c_arg(z)/(2.0*PI);
  float saturation = 0.59/sqrt(c_abs(z));

  if (c_abs(z) < 0.1) {
    saturation = 0.0;
  }

  float vibrance = 0.95 * max(1.0-float(steps)*0.025, 0.0);
